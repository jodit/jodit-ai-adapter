...
function adaptContext(
	...
	// The validated context structure matches IAIRequestContext exactly,
	// but TypeScript can't infer this from Zod schemas that use z.unknown() for nested arrays
	// We validate with Zod, then safely adapt the type since structure is guaranteed
	return {
		mode: validatedContext.mode,
		conversationId: validatedContext.conversationId,
...
export function createApp(config: AppConfig): Express {
	...
	app.post(
		...
		asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
			...
			// Get user's API key from request (set by auth middleware)
